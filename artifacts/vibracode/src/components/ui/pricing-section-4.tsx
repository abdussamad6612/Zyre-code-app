...
export default function PricingSection6() {
  ...
  return (
    <div className="min-h-screen w-full mx-auto relative bg-black overflow-x-hidden pt-20" ref={pricingRef}>
      <TimelineContent animationNum={4} timelineRef={pricingRef} customVariants={revealVariants} className="absolute top-0 h-96 w-screen overflow-hidden [mask-image:radial-gradient(50%_50%,white,transparent)] z-10">
        <div className="absolute bottom-0 left-0 right-0 top-0 bg-[linear-gradient(to_right,#ffffff2c_1px,transparent_1px),linear-gradient(to_bottom,#3a3a3a01_1px,transparent_1px)] bg-[size:70px_80px]"></div>
        <SparklesComp density={1800} speed={1} color="#FFFFFF" className="absolute inset-x-0 bottom-0 h-full w-full [mask-image:radial-gradient(50%_50%,white,transparent_85%)]" />
      </TimelineContent>
      <TimelineContent animationNum={5} timelineRef={pricingRef} customVariants={revealVariants} className="absolute left-0 top-[-114px] w-full h-[113.625vh] flex flex-col items-start justify-start content-start flex-none flex-nowrap gap-2.5 overflow-hidden p-0 z-0">
        <div className="framer-1i5axl2">
          <div className="absolute left-[-568px] right-[-568px] top-0 h-[2053px] flex-none rounded-full" style={{ border: "200px solid #3131f5", filter: "blur(92px)", WebkitFilter: "blur(92px)" }} data-border="true" />
          <div className="absolute left-[-568px] right-[-568px] top-0 h-[2053px] flex-none rounded-full" style={{ border: "200px solid #3131f5", filter: "blur(92px)", WebkitFilter: "blur(92px)" }} data-border="true" />
        </div>
      </TimelineContent>
      ...
      <div className="absolute top-0 left-[10%] right-[10%] w-[80%] h-full z-0" style={{ backgroundImage: `radial-gradient(circle at center, #206ce8 0%, transparent 70%)`, opacity: 0.6, mixBlendMode: "multiply" }} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 max-w-7xl gap-3 py-6 mx-auto px-4">
        {paidPlans && paidPlans.length > 0 && paidPlans.map((plan, index) => (
          <TimelineContent key={plan.id} as="div" animationNum={2 + index} timelineRef={pricingRef} customVariants={revealVariants}>
            <Card className={`relative text-white border-neutral-800 ${plan.id === 'pro' ? "bg-gradient-to-r from-neutral-900 via-neutral-800 to-neutral-900 shadow-[0px_-13px_300px_0px_#0900ff] z-20" : "bg-gradient-to-r from-neutral-900 via-neutral-800 to-neutral-900 z-10"}`}>
              <CardHeader className="text-left p-4">
                ...
                <div className="flex items-baseline">
                  <span className="text-3xl font-semibold">$
                    <NumberFlow format={{ currency: "USD" }} value={isYearly ? Math.round(plan.price * 10) : plan.price} className="text-3xl font-semibold" />
                  </span>
                  <span className="text-gray-300 ml-1 text-sm">/{isYearly ? "year" : plan.id === 'weekly_plus' ? "week" : "month"}</span>
                </div>
              ...
              <CardContent className="pt-0 p-4">
                <button
                  ...
                  className={`w-full mb-4 p-3 text-lg rounded-xl transition-all duration-200 ${plan.id === 'pro' ? "bg-gradient-to-t from-blue-500 to-blue-600 shadow-lg shadow-blue-800 border border-blue-500 text-white hover:from-blue-600 hover:to-blue-700" : "bg-gradient-to-t from-neutral-950 to-neutral-600 shadow-lg shadow-neutral-900 border border-neutral-800 text-white hover:from-neutral-800 hover:to-neutral-500"}`}
                >
                  {plan.id === 'weekly_plus' ? 'Start Weekly Trial' : plan.id === 'pro' ? 'Get Started' : plan.id === 'business' ? 'Upgrade to Business' : 'Get Started'}
                </button>
